...
export const CREDIT_SETTING_DEFINITIONS = [
  {
    key: "p_credit_value_minor",
    label: "P-credit value (minor units)",
    type: "integer",
    description: "Official reporting/exchange value for one P-credit.",
  },
  {
    key: "r_credit_value_minor",
    label: "R-credit value (minor units)",
    type: "integer",
    description: "Reference value for one R-credit; R-credit remains non-cash.",
  },
  {
    key: "payout_currency",
    label: "Payout currency",
    type: "currency",
    description: "Three-letter currency used by exchange rates.",
  },
  {
    key: "p_exchange_min_credits",
    label: "P-credit exchange minimum",
    type: "integer",
    description: "Minimum P-credit amount per exchange request.",
  },
  {
    key: "p_exchange_max_credits",
    label: "P-credit exchange maximum",
    type: "nullable_integer",
    description: "Maximum P-credit amount; zero means no maximum.",
  },
  {
    key: "r_exchange_min_credits",
    label: "R-credit exchange minimum",
    type: "integer",
    description: "Minimum non-cash R-credit exchange amount.",
  },
  {
    key: "r_exchange_max_credits",
    label: "R-credit exchange maximum",
    type: "nullable_integer",
    description: "Maximum non-cash R-credit amount; zero means no maximum.",
  },
  {
    key: "payout_mechanism",
    label: "Payout mechanism",
    type: "text",
    description: "Configured settlement route, for example manual bank transfer.",
  },
  {
    key: "payout_enabled",
    label: "Cash payout enabled",
    type: "boolean",
    description: "Enables cash payout only for P-credit.",
  },
  {
    key: "reconciliation_adapter",
    label: "Reconciliation adapter",
    type: "text",
    description: "Adapter or operating process used to reconcile settlement.",
  },
  {
    key: "reconciliation_export_format",
    label: "Reconciliation export format",
    type: "format",
    description: "Format used by finance reconciliation exports.",
  },
  {
    key: "reconciliation_cycle_days",
    label: "Reconciliation cycle (days)",
    type: "integer",
    description: "Rolling bank reconciliation cycle length.",
  },
  {
    key: "budget_minor_units",
    label: "Approved budget (minor units)",
    type: "nullable_integer",
    description: "Approved budget; zero means not specified.",
  },
  {
    key: "default_r_issuance_policy",
    label: "Default R-credit issuance policy",
    type: "issuance_policy",
    description: "Controls how recognition-credit issuance is governed.",
  },
  {
    key: "standing_occasion_approval",
    label: "Standing occasion approval",
    type: "approval_policy",
    description: "Controls whether standing-occasion grants require approval.",
  },
  {
    key: "employee_fulfillment_visibility",
    label: "Employee fulfillment visibility",
    type: "visibility",
    description: "Controls whether employees can see their reward fulfillment status.",
  },
] as const;
...
export const CREDIT_SETTING_DEFAULTS: Record<CreditSettingKey, unknown> = {
  // Finance must approve the value before Exchange is enabled. Zero means not configured.
  p_credit_value_minor: 0,
...
export function defaultCreditSettingPermission(
  key: CreditSettingKey,
  role: AdminRole,
...
export function normalizeCreditSettingValues(
  values: Record<string, unknown>,
): Record<CreditSettingKey, unknown> {
  ...
  for (const [key, rawValue] of Object.entries(values)) {
    ...
    if (definition.type === "integer" || definition.type === "nullable_integer") {
      if (
        definition.type === "nullable_integer" &&
    ...
    if (
      ...
      !["REASON_REQUIRED", "APPROVAL_REQUIRED"].includes(value)
    ) {
      throw new LoyaltyError("CREDIT_SETTING_INVALID_VALUE", 400, { key });
    }
    ...
    CREDIT_SETTING_DEFINITIONS.map((definition) =>
      db.creditSetting.upsert({
        ...
        create: {
          programId,
          key: definition.key,
          value: defaults[definition.key] as Prisma.InputJsonValue,
        },
        update: {},
      }),
    ),
...
export async function getCreditSettings(programId: string, db: PrismaClient = prisma) {
  const [rows, permissions] = await Promise.all([
    ...
    db.creditSettingPermission.findMany({
      where: { programId },
      orderBy: [{ settingKey: "asc" }, { role: "asc" }],
    }),
  ]);
...
export async function requireCreditSettingPermission(
  ...
  const permission = await db.creditSettingPermission.findUnique({
    where: { programId_settingKey_role: { programId, settingKey: key, role: admin.role } },
  });
  const allowed =
    permission?.[action === "VIEW" ? "canView" : "canEdit"] ??
    defaultCreditSettingPermission(definition.key, admin.role)[
      action === "VIEW" ? "canView" : "canEdit"
    ];
...
export async function requireSuperAdmin(
  ...
  if (!admin || admin.programId !== programId || admin.role !== "SUPER_ADMIN")
...
export async function syncExchangeRatesFromSettings(
  programId: string,
  values: Record<string, unknown>,
  db: PrismaClient = prisma,
): Promise<void> {
  const currency = String(values.payout_currency ?? CREDIT_SETTING_DEFAULTS.payout_currency);
  const payoutMechanism = String(
    values.payout_mechanism ?? CREDIT_SETTING_DEFAULTS.payout_mechanism,
  );
  const payoutEnabled = values.payout_enabled === true;
  const definitions: Array<{
    creditType: "P" | "R";
    valueKey: CreditSettingKey;
    minKey: CreditSettingKey;
    maxKey: CreditSettingKey;
  }> = [
    {
      creditType: "P",
      valueKey: "p_credit_value_minor",
      minKey: "p_exchange_min_credits",
      maxKey: "p_exchange_max_credits",
    },
    {
      creditType: "R",
      valueKey: "r_credit_value_minor",
      minKey: "r_exchange_min_credits",
      maxKey: "r_exchange_max_credits",
    },
  ];
  for (const definition of definitions) {
    ...
    const active = await db.creditExchangeRate.findFirst({
      where: { programId, creditType: definition.creditType, isActive: true },
      orderBy: { version: "desc" },
    });
    const unchanged =
      active &&
      active.valueMinorPerCredit === valueMinorPerCredit &&
      active.currency === currency &&
      active.payoutMechanism === payoutMechanism &&
      active.cashEligible === (definition.creditType === "P" && payoutEnabled) &&
      active.minCredits === minCredits &&
      active.maxCredits === maxCredits;
    ...
    await db.$transaction(async (tx) => {
      ...
      const latest = await tx.creditExchangeRate.findFirst({
        where: { programId, creditType: definition.creditType },
        orderBy: { version: "desc" },
        select: { version: true },
      });
