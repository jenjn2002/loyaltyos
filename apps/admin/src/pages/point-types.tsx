    ...
    <div className="space-y-6 pb-10">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-bold">
            <Settings2 className="h-6 w-6" /> Point type configuration
          </h1>
          <p className="mt-1 max-w-3xl text-sm text-muted-foreground">
            Every wallet uses this single engine. Start blank, create any point type, or explicitly
            apply the optional P/R template. Types with history are archived; unused types are
            deleted.
          </p>
        </div>
        <Button
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            P and R have no special lock. Delete any unused type; a used type is safely archived and
            removed from member profiles.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          {!pointTypes.isLoading && (pointTypes.data ?? []).length === 0 && (
            <p className="rounded-md border border-dashed p-6 text-center text-sm text-muted-foreground">
              No point types configured. Create one above or apply the optional P/R template.
            </p>
          )}
