...
Shared i18n package for LoyaltyOS. Provides locale catalogs (vi-VN as default, en-US),
translation helpers, date/currency/number formatting, and locale detection.

## Adding a new locale

1. Copy `src/locales/vi-VN.json` to `src/locales/<new-locale>.json` and translate all values
...
- `campaigns.typeLabels.BONUS_POINTS` → "Điểm thưởng" / "Bonus Points"
- `errors.INSUFFICIENT_BALANCE` → "Saldo insuficiente" / "Insufficient balance"

## Adding a new key

1. Add the key to `src/locales/vi-VN.json` first (this is the source of truth)
...
// Format
formatDate(new Date(), "vi-VN"); // "27 tháng 5, 2026"
formatCurrency(19.99, "USD", "en-US"); // "$19.99"
formatPoints(1500, "vi-VN"); // "1,500 pts"
